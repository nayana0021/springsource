...
//		list.stream().forEach(null);
		list.forEach(item -> System.out.println(item)); // 위와 같이 쓸수있지만 foreach를 바로 쓸수도 있다 // 변수 이름이 item 
...
//		}
		...
		// Map
		Map<String, Integer> map = new HashMap<String, Integer>(); // key:value 형태 구조 키값은 중복이 되면 안되고 밸류는 상관없음
		map.put("A", 10);
		map.put("B", 20);
		map.put("C", 30);
		map.put("D", 40);
