...
document.querySelector("#wishButton").addEventListener("click", () => {
  ...
  fetch("/api/restaurant", {
    ...
    body: JSON.stringify(wishItem), //JSON 상태로 보내줘
  })
    .then((response) => response.json()) // null 값도 들어가는 상황이고 딱히 오류날 상황이 아니어서 바로 json으로 받음
    .then((data) => console.log(data)) // then 부분은 데이터가 넘어와서 작업하는 부분임
    .catch((error) => console.log(error));
});
...
function showList() {
  fetch("/api/restaurant/all")
    .then((response) => {
      ...
      return response.json(); // 넘어오는 데이터 json으로 바꿔줘
    })
    .then((data) => {
      console.log(data);
